...
# ─────────────────────────────────────────────────────────────────────────────
# SESSION STATE INIT (AJUSTE NAVEGAÇÃO)
# ─────────────────────────────────────────────────────────────────────────────
...
if "nav_sidebar_select" not in st.session_state:
    st.session_state["nav_sidebar_select"] = st.session_state["pagina_atual"]
...
# ─────────────────────────────────────────────────────────────────────────────
# SISTEMA DE LOCK INTELIGENTE (SEM TRAVAMENTO PERMANENTE)
# ─────────────────────────────────────────────────────────────────────────────

if "locks_empresa" not in st.session_state:
    st.session_state["locks_empresa"] = {}

def limpar_locks_expirados():
    agora = time.time()
    for empresa in list(st.session_state.get("locks_empresa", {}).keys()):
        ts = st.session_state["locks_empresa"].get(empresa)

        if ts is None:
            continue

        if agora - ts > LOCK_TIMEOUT:
            del st.session_state["locks_empresa"][empresa]

def bloquear_empresa(nome):
    limpar_locks_expirados()
    st.session_state["locks_empresa"][nome] = time.time()


def liberar_empresa(nome):
    limpar_locks_expirados()
    if nome in st.session_state.get("locks_empresa", {}):
        del st.session_state["locks_empresa"][nome]


def empresa_bloqueada(nome):
    limpar_locks_expirados()
    return nome in st.session_state.get("locks_empresa", {})
...
# ─────────────────────────────────────────────────────────────────────────────
# NAVEGAÇÃO (VERSÃO ESTÁVEL FINAL + LOCK INTELIGENTE)
# ─────────────────────────────────────────────────────────────────────────────
...
if st.session_state["pagina_atual"] not in perfis_navegacao:
    st.session_state["pagina_atual"] = perfis_navegacao[0]

_idx_atual = perfis_navegacao.index(st.session_state["pagina_atual"])
...
# ─────────────────────────────────────────────────────────────────────────────
# CONTROLE DE NAVEGAÇÃO + LOCK DE EMPRESAS
# ─────────────────────────────────────────────────────────────────────────────
...
pagina_anterior = st.session_state["pagina_atual"]

# 🔁 só processa mudança se houver troca real
if pagina_escolhida != pagina_anterior:

    # 🔥 SAINDO DE EMPRESA → LIBERA LOCK
    if pagina_anterior in EMPRESAS_TELAS:
        liberar_empresa(pagina_anterior)

    # 🔥 ENTRANDO EM EMPRESA → VERIFICA LOCK
    if pagina_escolhida in EMPRESAS_TELAS:

        if empresa_bloqueada(pagina_escolhida):
            st.warning("🔒 Empresa em uso por outro acesso. Aguarde alguns segundos.")
            st.stop()

        bloquear_empresa(pagina_escolhida)

    # 🔥 SAINDO DO GERENCIADOR → REMOVE ACESSO ADMIN
    if pagina_anterior == "🎛️ Painel Gerenciador":
        st.session_state["gerenciador_autenticado"] = False
...
# estado final sempre consistente
perfil = st.session_state["pagina_atual"]

# ─────────────────────────────────────────────────────────────────────────────
# (TODO O RESTO DO SEU CÓDIGO PERMANECE IGUAL ABAIXO)
# ─────────────────────────────────────────────────────────────────────────────
...
def gerar_carta_destino(nome: str, r1, r2, r3) -> str:
    ...
    if qtd_c == 3:
        ...
        texto = (
            f"A investigação concluiu que houve apenas um episódio isolado de gerenciamento de resultados por accruals discricionários. 📚\n\n"
...
def gerar_manchete_dinamica(estado: dict, rodada_encerrada: int) -> str:
    ...
    if todos_empatados:
        ...
        <div style="background-color:#2e7d32;color:#fff;padding:10px 15px;border-radius:2px;
            ...
            RODADA ENCERRADA SEM VARIAÇÕES EXTREMAS
        </div>
        <div style="margin-top:6px;border-left:4px solid #2e7d32;
                    padding:8px 12px;background-color:#e8f5e9;">
            <p style="font-size:13px;color:#333;margin:0;text-align:justify;">
                Mercado estável nesta rodada.
            </p>
        </div>
        """
        ...
        topo_texto = (
            f"SÃO PAULO — A {lider_nome} subiu de R$ "
            f"{lider_dados['anterior']:.2f} para R$ {lider_dados['atual']:.2f}."
        )

        baixo_manchete = f"{lanterna_nome} despenca {fmt_var(lanterna_dados['var'])}"
        baixo_texto = (
            f"SÃO PAULO — A {lanterna_nome} caiu para R$ "
...
if perfil == "🏠 Início":
    ...
    # 🔒 LOCK GLOBAL (garante existência da chave)
    if "sessoes_ativas" not in estado:
        estado["sessoes_ativas"] = []
        salvar_estado(estado)
    ...
    with c2:
        with st.container(border=True):
            ...
            if st.button("Entrar como representante da empresa", use_container_width=True, type="primary"):
                ...
                # 🔒 LOCK: evita duplicação e garante consistência
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    if not estado.get("historico_noticias_plantao"):
        estado["historico_noticias_plantao"] = []
...
# Timer — botão de início + exibição
    ...
    # 🔒 LOCK: garante estrutura consistente
    if "historico_noticias_plantao" not in estado or estado["historico_noticias_plantao"] is None:
        estado["historico_noticias_plantao"] = []
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    # 🔒 LOCKS de segurança estrutural
    if "historico_noticias" not in estado or estado["historico_noticias"] is None:
        estado["historico_noticias"] = []
    ...
    if estado["historico_noticias_plantao"]:
        st.markdown(estado["historico_noticias_plantao"][-1], unsafe_allow_html=True)
    ...
    if (
        ...
        and not estado["historico_noticias_plantao"]
        and not estado["historico_noticias_veredicto"]
    ):
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    # 🔒 LOCK GLOBAL DE SESSÕES
    if "sessoes_ativas" not in estado or estado["sessoes_ativas"] is None:
        estado["sessoes_ativas"] = []

    sessoes_ativas = estado["sessoes_ativas"]

    chave_auth = f"auth_{nome_interno}"

    if not st.session_state.get(chave_auth, False):

        # Verifica se já está ocupada por outro dispositivo
        ...
        if st.button("Entrar", use_container_width=True, type="primary"):
            if senha_input == SENHAS_EMPRESAS.get(nome_interno, ""):
                ...
                # 🔒 registra sessão com proteção contra duplicação
