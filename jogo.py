...
# ─────────────────────────────────────────────────────────────────────────────
# 2. Banco de Dados Global Real-Time (Corrigido para versões novas do Streamlit)
# ─────────────────────────────────────────────────────────────────────────────
...
def obter_conexao_banco_global():
    """Garante uma única instância compartilhada por todos os usuários do servidor"""
...
# ═════════════════════════════════════════════════════════════════════════════
# VISÃO DO ALUNO: TOTALMENTE ISOLADA
# ═════════════════════════════════════════════════════════════════════════════
        ...
        if not voto_ja_realizado:
            ...
            if rodada == 1:
                ...
                with st.expander("📌 **Opção B — Detalhes Técnicos**"):
                    ...
                    st.markdown("**Justificativa:** A extensão de prazo é uma condition comercial do setor e o risco principal permanece ligado à entrega da mercadoria, mantendo a natureza operacional do passivo.")
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    if db.rodada_atual == 4:
        ...
        for nome, d in db.dados_empresas.items():
            st.info(f"**{nome}** — {d['status']} | **Preço de Fechamento:** R$ {d['precos'][-1]:.2f}")import streamlit as st
import matplotlib.pyplot as plt
